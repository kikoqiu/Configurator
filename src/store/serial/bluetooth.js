...
class BT extends EventTarget {
    ...
    handleNewDevice(device) {
        ...
        this.dispatchEvent(new CustomEvent("connect", { detail:added }));
    ...
    handleRemovedDevice(device) {
        ...
        this.dispatchEvent(new CustomEvent("disconnect", { detail: removed }));
    }
    ...
    async connect(path, options) {
        ...
        try {
            ...
            console.log(`${this.logHead} Connected to GATT Server`);
            ...
            console.log(`${this.logHead} failed to Connect to GATT Server`);
        }
    ...
    async read(chunkSize){
        while(this.buffer.length==0){
            if(!this.bufferPromise){
                this.bufferPromise=new Promise((rs,rj)=>{
                    this.bufferResolve=rs;
                    this.bufferReject=rj;
                });
            }
        ...
        let first=this.buffer.shift();
        if(first.length<=chunkSize){
            return {data:first};
        }else{
            let ret=first.subarray(0,chunkSize);
            let left=first.subarray(chunkSize);
            this.buffer.unshift(left);
            return {data:ret};
        }
    }
    ...
    handleNotification(event) {
        ...
        //console.log(`get ${event.target.value.byteLength}`);
